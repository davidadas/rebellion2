...
{
    /// <summary>
    /// Holds resolved message content and its transient delivery context.
    /// A supplied report receives the content only when delivery reaches this item.
    /// </summary>
    public sealed class MessageDelivery
    {
        public Faction Recipient { get; set; }
        public Message Message { get; set; }
        public Message ExistingMessage { get; set; }
    ...
    public partial class MessageFactory
    {
        ...
        /// <summary>
        /// Constructs a resolved message or applies its presentation to a supplied report.
        /// This does not attach the message to a faction or assign its delivery tick.
        /// </summary>
        /// <param name="delivery">The resolved text, presentation, and optional report.</param>
        /// <returns>The message ready for delivery.</returns>
        public Message CreateMessage(MessageDelivery delivery)
        {
            Message content = delivery.Message;
            Message message = delivery.ExistingMessage ?? content;
            message.Type = content.Type;
            message.ResultType = content.ResultType;
            message.Title = content.Title;
            message.Body = content.Body;
            message.DisplayName = content.Title;
            message.BackgroundImageKey = content.BackgroundImageKey;
            message.DisplayImagePath = content.DisplayImagePath;
            message.OverlayImagePath = content.OverlayImagePath;
            message.BackgroundAudioPath = content.BackgroundAudioPath;
            message.OfficerVoicePath = content.OfficerVoicePath;
            message.EventLocationInstanceID = content.EventLocationInstanceID;
            message.NavigationTargetInstanceID = content.NavigationTargetInstanceID;
            message.NavigationSecondaryTargetInstanceID =
                content.NavigationSecondaryTargetInstanceID;
            message.MissionInstanceID = content.MissionInstanceID;
            return message;
        }
        ...
        public List<MessageDelivery> CreateMessages(IEnumerable<GameResult> results, GameRoot game)
        {
            ...
            GameObjectSabotagedResult[] sabotageResults = batch
                ...
                .ToArray();
            PlanetsRevealedResult[] planetIntelligenceResults = batch
                .OfType<PlanetsRevealedResult>()
                .ToArray();
        ...
        )
        {
            foreach (TraitorDiscoveredResult result in results)
            {
                ...
                MessageDelivery message = WithAdvisorSubject(
                    WithEventLocation(
                        CreateMessage(
        ...
        private MessageDelivery CreateEmperorSeatOfPower(Faction faction, Officer officer)
        {
        ...
        /// <param name="planetIntelligence">Additional planets revealed by this mission.</param>
        /// <returns>The mission report message, or null when no matching definition exists.</returns>
        private MessageDelivery CreateMissionReport(
            Faction faction,
            ...
            PlanetsRevealedResult planetIntelligence
        )
        {
            if (result == null)
                return null;
            ...
            if (message != null && result.CanContinue)
                message.Message.MissionInstanceID = result.MissionInstanceID;
        ...
        /// <summary>
        /// Builds a configured mission-detail list for planets revealed beyond the primary target.
        /// </summary>
        ...
        {
            Planet[] planetArray = planets?.Where(planet => planet != null).ToArray();
            if (definition == null || planetArray == null || planetArray.Length == 0)
                return string.Empty;
        ...
        private MessageDelivery CreateEnemyMissionFoiled(
            Faction faction,
            MissionCompletedResult result,
            Planet target
        )
        ...
        private MessageDelivery CreateOfficerMessage(
        ...
        private MessageDelivery CreateForceGrowth(
            Faction faction,
        ...
        private MessageDelivery CreateForceUserDiscovered(
            Faction faction,
        ...
        private MessageDelivery CreateSabotageStrike(
            Faction faction,
        ...
        private MessageDelivery CreateRecruitmentExhausted(RecruitmentExhaustedResult result)
        {
        ...
        /// <param name="planetIntelligenceResults">Additional-planet intelligence results.</param>
        ...
        private void AddMissionMessages(
            ...
            IEnumerable<OfficerKilledResult> killedResults,
            IEnumerable<GameObjectSabotagedResult> sabotageResults,
            IEnumerable<PlanetsRevealedResult> planetIntelligenceResults,
            GameRoot game,
            List<MessageDelivery> deliveries
        )
        {
            ...
            HashSet<string> killedOfficerIDs = killedArray
                ...
                .ToHashSet();
            Dictionary<string, PlanetsRevealedResult> planetIntelligenceByMission = (
                planetIntelligenceResults ?? Array.Empty<PlanetsRevealedResult>()
            )
            ...
            foreach (MissionCompletedResult result in results)
            {
                ...
                planetIntelligenceByMission.TryGetValue(
                    result.MissionInstanceID ?? string.Empty,
                    out PlanetsRevealedResult planetIntelligence
                );
                AddDelivery(
                    deliveries,
                    actorFaction,
        ...
        private MessageDelivery WithAdvisorNotification(
            MessageDelivery message,
        ...
        private MessageDelivery WithAdvisorSubject(
            MessageDelivery message,
        ...
        private static MessageDelivery WithEventLocation(
            MessageDelivery message,
            Planet planet,
        ...
        )
        {
            if (message != null)
            {
                message.Message.EventLocationInstanceID = planet?.GetInstanceID();
                message.Message.NavigationTargetInstanceID = (target ?? planet)?.GetInstanceID();
                message.Message.NavigationSecondaryTargetInstanceID =
                    secondaryTarget?.GetInstanceID();
            }
    ...
    public partial class MessageFactory
    {
        ...
        private MessageDelivery CreateFleet(Faction faction, Fleet fleet, Planet destination)
        {
            MessageDelivery message = BuildArrivalMessage(
        ...
        /// <param name="deliveries">The collection receiving prepared messages.</param>
        ...
        private MessageDelivery CreateShips(
            Faction faction,
        ...
        private MessageDelivery CreateUnits(
            Faction faction,
        ...
        private MessageDelivery CreatePersonnel(
            Faction faction,
        ...
        private MessageDelivery CreateHeadquarters(
            Faction faction,
        ...
        private MessageDelivery BuildArrivalMessage(
            MessageResultType resultType,
            Faction faction,
            Dictionary<string, string> values,
            string overlayImagePath = null,
        ...
        {
            MessageDelivery message = _templateBuilder.Build(
                definition,
                faction,
                values,
        ...
        )
        {
            if (message == null)
                return;
            message.Message.EventLocationInstanceID = planet?.InstanceID;
            message.Message.NavigationTargetInstanceID = (target ?? planet)?.InstanceID;
        }
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            ...
            foreach (EvacuationLossesResult result in evacuationResults)
            {
                ...
                MessageDelivery message = BuildBlockadeMessage(
                    definition,
                    result.Faction,
                    new Dictionary<string, string>
                    {
        ...
        private MessageDelivery BuildBlockadeMessage(
            MessageDefinition definition,
            Faction recipient,
            Dictionary<string, string> values,
            Faction imageFaction = null
    ...
    public partial class MessageFactory
    {
        ...
        private MessageDelivery CreateSpaceBattle(
            Faction faction,
        ...
        )
        {
            ...
            MessageDelivery message = BuildCombatMessage(definition, faction, values);
        ...
        private MessageDelivery CreateBombardment(
            Faction faction,
        ...
        )
        {
            ...
            MessageDelivery message = BuildCombatMessage(
                definition,
                faction,
        ...
        private MessageDelivery CreateAssault(
            Faction faction,
        ...
        private MessageDelivery BuildCombatMessage(
            MessageDefinition definition,
            Faction faction,
            Dictionary<string, string> values,
            Faction imageFaction = null
        ...
        )
        {
            if (message == null)
                return;
            message.Message.EventLocationInstanceID = planet?.InstanceID;
            message.Message.NavigationTargetInstanceID = target?.InstanceID ?? planet?.InstanceID;
        }

        /// <summary>
        /// Creates the combat report delivered by a combat-result delivery.
        /// </summary>
        /// <param name="delivery">The prepared delivery that receives the combat report.</param>
        ...
        )
        {
            ...
            delivery.ExistingMessage = CombatReport.Capture(
                result,
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            ...
            )
            {
                ...
                MessageDelivery message = unit is Building building
        ...
        )
        {
            foreach (GameObjectDestroyedOnArrivalResult result in results)
            {
                ...
                MessageDelivery message = BuildDeploymentMessage(
                    definition,
                    faction,
                    new Dictionary<string, string>
                    {
        ...
        public MessageDelivery CreateFacilityMessage(
            Faction faction,
            Building building,
            Planet destination
        )
        ...
        private MessageDelivery CreateUnit(Faction faction, IGameEntity unit, Planet destination)
        {
        ...
        private MessageDelivery CreateRegiments(
            Faction faction,
        ...
        private MessageDelivery BuildDeploymentMessage(
            MessageDefinition definition,
            Faction faction,
            Dictionary<string, string> values,
            string imageOverride = null
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            foreach (ManufacturingIdleResult result in results)
            {
                ...
                MessageDelivery message = BuildEconomyMessage(
                    GetDefinition(
                ...
                if (message != null)
                {
                    message.Message.EventLocationInstanceID = result.ProductionPlanet?.InstanceID;
                    message.Message.NavigationTargetInstanceID = result
                        .ProductionPlanet
                        ?.InstanceID;
                }
        ...
        )
        {
            ...
            if (message != null)
            {
                message.Message.EventLocationInstanceID = result.Planet?.InstanceID;
                message.Message.NavigationTargetInstanceID = result.Planet?.InstanceID;
            }
            AddDelivery(deliveries, recipient, message);
        }
        ...
        private MessageDelivery BuildEconomyMessage(
            MessageDefinition definition,
            Faction faction,
            Dictionary<string, string> values
        )
        {
            MessageDelivery message = _templateBuilder.Build(definition, faction, values);
            return WithAdvisorNotification(
                message,
                AdvisorNotificationPolicy.GetDefault(definition?.ResultType)
            );
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            ...
            )
            {
                ...
                if (message != null)
                {
                    message.Message.EventLocationInstanceID = first.Location?.InstanceID;
                    message.Message.NavigationTargetInstanceID =
    ...
    public partial class MessageFactory
    {
        ...
        private MessageDelivery CreateNearUprising(Faction faction, PlanetNearUprisingResult result)
        {
            if (result == null)
                return null;
        ...
        private MessageDelivery CreateUprisingStarted(
            Faction faction,
        ...
        private MessageDelivery CreateUprisingEnded(
            Faction faction,
        ...
        private MessageDelivery CreateJoined(PlanetOwnershipChangedResult result)
        {
        ...
        private MessageDelivery CreateJoinedEnemy(
        ...
        private MessageDelivery CreateNeutrality(
            PlanetOwnershipChangedResult result,
            Faction recipient
        )
        {
        ...
        private MessageDelivery BuildPoliticalMessage(
            MessageResultType resultType,
            Faction faction,
            Dictionary<string, string> values,
        ...
        )
        {
            ...
            if (message != null)
            {
                message.Message.EventLocationInstanceID = planetInstanceID;
                message.Message.NavigationTargetInstanceID = planetInstanceID;
            }
            if (message != null)
                message.NotificationType = notification;
            return message;
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            ...
            if (message != null)
            {
                message.Message.EventLocationInstanceID =
                    unit.GetParentOfType<Planet>()?.InstanceID;
                message.Message.NavigationTargetInstanceID = unit.InstanceID;
            }
            WithAdvisorNotification(
                message,
                AdvisorNotificationPolicy.GetDefault(definition?.ResultType)
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            foreach (ResearchOrderedResult result in completedResults)
            {
                ...
                MessageDelivery message = BuildResearchMessage(
                    GetDefinition(
        ...
        private MessageDelivery BuildResearchMessage(
            MessageDefinition definition,
            Faction faction,
            Dictionary<string, string> values
        )
    ...
    public partial class MessageFactory
    {
        ...
        )
        {
            foreach (PlanetOwnershipChangedResult result in ownershipResults)
            {
                ...
                {
                    MessageDelivery message = BuildStrategicMessage(
                        definition,
                        recipient,
                        new Dictionary<string, string>
                        {
        ...
        private MessageDelivery BuildStrategicMessage(
            MessageDefinition definition,
            Faction recipient,
            Dictionary<string, string> values,
        ...
        /// <summary>
        /// Resolves authored text against the selected subjects and location without delivering it.
        /// </summary>
        ...
        /// <returns>The prepared delivery, or null when no recipient or definition is supplied.</returns>
        public MessageDelivery CreateAuthoredMessage(
            MessageDefinition definition,
            Faction recipient,
        ...
        )
        {
            ...
            MessageDelivery message = _templateBuilder.Build(
                ...
                new Dictionary<string, string>
                {
                    { "subject", subject?.GetDisplayName() ?? string.Empty },
                    { "relatedSubject", relatedSubject?.GetDisplayName() ?? string.Empty },
                    { "location", location?.GetDisplayName() ?? string.Empty },
                    { "faction", recipient.GetDisplayName() },
                },
                overlayImagePath: overlayImagePath
            );
            if (message == null)
            ...
            message.Message.EventLocationInstanceID = location?.InstanceID;
            message.Message.NavigationTargetInstanceID = subject?.InstanceID;
            message.AdvisorNotification = notification;
            message.AdvisorSubjectTypeID = subject?.TypeID;
            message.SourceEventInstanceID = sourceEventInstanceID;
            ApplyAdvisorPreset(message);
            return message;
        }
