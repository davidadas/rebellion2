...
namespace Rebellion.Simulation
{
    ...
    /// </summary>
    internal sealed class Duels : IGameCommandHandler<DuelCommand>
    {
        ...
        public Duels(GameRoot game, IRandomNumberProvider random)
        {
        ...
        public List<GameResult> HandleCommands(IReadOnlyList<DuelCommand> results)
        {
            List<GameResult> reactions = new List<GameResult>();
            if (results == null)
                return reactions;

            foreach (DuelCommand request in results)
            {
        ...
        private static bool CanResolveDuel(DuelCommand request)
        {
        ...
        private void ResolveDuel(DuelCommand request, List<GameResult> reactions)
        {
            Officer encountered = request.EncounteredOfficer;
            Officer opposing = request.OpposingOfficer;
            Planet location = encountered.GetParentOfType<Planet>();
        ...
        private static T Stamp<T>(T reaction, DuelCommand request)
            where T : GameResult
        {
