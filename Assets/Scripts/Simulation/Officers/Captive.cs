...
namespace Rebellion.Simulation
{
    ...
    /// </summary>
    internal class Captives
        : IGameCommandHandler<SetCaptureStatusCommand>,
            ...
            IGameResultHandler<PlanetOwnershipChangedResult>
    {
        ...
        /// <summary>
        /// Applies an authored capture state and reports each changed officer.
        /// </summary>
        /// <param name="commands">The capture-state commands.</param>
        /// <returns>The resulting capture-state facts.</returns>
        public List<GameResult> HandleCommands(IReadOnlyList<SetCaptureStatusCommand> commands)
        {
            List<GameResult> results = new List<GameResult>();
            foreach (
                SetCaptureStatusCommand command in commands
                    ?? Array.Empty<SetCaptureStatusCommand>()
            )
            {
                foreach (
                    Officer officer in command?.Officers?.Where(officer => officer != null)
                        ?? Enumerable.Empty<Officer>()
                )
                {
                    officer.IsCaptured = command.IsCaptured;
                    officer.CaptorInstanceID = command.IsCaptured
                        ? command.CaptorFactionInstanceID
                        : null;
                    officer.CanEscape = command.IsCaptured ? command.CanEscape : true;
                    results.Add(
                        new OfficerCaptureStateResult
                        {
                            TargetOfficer = officer,
                            IsCaptured = command.IsCaptured,
