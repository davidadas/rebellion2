...
namespace Rebellion.Simulation
{
    ...
    /// </summary>
    internal class GameEvents : IGameResultHandler<GameResult>
    {
        ...
        /// <param name="commands">Executes action commands through authoritative features.</param>
        public GameEvents(
            GameRoot game,
            IRandomNumberProvider provider,
            UnitFactory unitFactory = null,
            IGameCommandExecutor commands = null
        )
        {
            _game = game;
            _provider = provider;
        ...
        )
        {
            ...
            if (actionContext.Commands.Count > 0)
            {
                if (_commands == null)
                    throw new InvalidOperationException(
                        $"Event '{gameEvent.InstanceID}' produced commands without a configured executor."
                    );
                results.AddRange(_commands.ExecuteRaw(actionContext.Commands));
            }
