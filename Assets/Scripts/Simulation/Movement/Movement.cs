...
namespace Rebellion.Simulation
{
    ...
    internal class Movement
        : IGameResultHandler<BlockadeChangedResult>,
            IGameCommandHandler<MoveUnitsCommand>,
            IGameCommandHandler<PlaceUnitsCommand>
    {
        private readonly GameRoot _game;
        private readonly FogOfWar _fogOfWar;
        private readonly Blockades _blockade;
        private readonly Fleets _fleetSystem;
        ...
        List<GameResult> IGameCommandHandler<MoveUnitsCommand>.HandleCommands(
            IReadOnlyList<MoveUnitsCommand> requests
        )
        {
            if (requests == null)
            ...
            List<GameResult> reactions = new List<GameResult>();
            foreach (MoveUnitsCommand result in requests)
            {
        ...
        /// <param name="requests">The requests.</param>
        /// <returns>The result of handle requests.</returns>
        List<GameResult> IGameCommandHandler<PlaceUnitsCommand>.HandleCommands(
            IReadOnlyList<PlaceUnitsCommand> requests
        )
        {
            if (requests == null)
                return new List<GameResult>();

            foreach (PlaceUnitsCommand result in requests)
            {
                if (result?.Units == null || result.Destinations == null)
                    continue;
