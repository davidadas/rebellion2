...
namespace Rebellion.Game
{
    ...
    public class Planet : ContainerNode
    {
        ...
        /// <summary>
        /// Calculates required garrison strength for this planet based on popular support.
        /// Uses faction-specific modifiers from the owning faction.
        /// </summary>
        /// <param name="game">Game instance to get faction data.</param>
        /// <param name="baseRequirement">Base garrison requirement from game config or formula.</param>
        /// <returns>Adjusted garrison requirement based on faction modifiers.</returns>
        public int CalculateGarrisonRequirement(GameRoot game, int baseRequirement)
        {
            if (string.IsNullOrEmpty(OwnerInstanceID))
            {
                return 0;
            ...
            Faction owner = game.GetFactionByOwnerInstanceID(OwnerInstanceID);
            if (owner == null)
            {
                return baseRequirement;
            }

            return baseRequirement / owner.Modifiers.GarrisonEfficiency;
        }

        /// <summary>
        /// Calculates effective troop strength for support calculations.
        /// Uses faction-specific troop effectiveness modifiers.
        /// </summary>
        /// <param name="game">Game instance to get faction data.</param>
        /// <param name="baseTroopCount">Base troop count.</param>
        /// <returns>Effective troop strength for support calculations.</returns>
        public int CalculateEffectiveTroopStrength(GameRoot game, int baseTroopCount)
        {
            if (string.IsNullOrEmpty(OwnerInstanceID))
            {
                return baseTroopCount;
            }
            ...
            return baseTroopCount * owner.Modifiers.TroopEffectiveness;
        }

        /// <summary>
        /// Applies faction-specific support shift inversion if needed.
        /// </summary>
        /// <param name="game">Game instance to get faction data.</param>
        /// <param name="supportShift">Base support shift amount.</param>
        /// <returns>Support shift adjusted for faction modifiers.</returns>
        public int ApplySupportShiftModifiers(GameRoot game, int supportShift)
        {
            if (string.IsNullOrEmpty(OwnerInstanceID))
            {
                return supportShift;
            ...
            if (owner.Modifiers.InvertSupportShift)
            {
        ...
        /// <summary>
        /// Checks if weak support penalty should apply for the owning faction.
        /// </summary>
        /// <param name="game">Game instance to get faction data.</param>
        /// <param name="supportShift">Current support shift value.</param>
        /// <returns>True if weak support penalty applies.</returns>
        public bool ShouldApplyWeakSupportPenalty(GameRoot game, int supportShift)
        {
            if (string.IsNullOrEmpty(OwnerInstanceID))
            {
                return false;
            ...
            return owner.Modifiers.WeakSupportPenaltyTrigger switch
            {
                SupportShiftCondition.Positive => supportShift > 0,
                SupportShiftCondition.Negative => supportShift < 0,
                _ => false,
            };
        }
