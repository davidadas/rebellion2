...
namespace Rebellion.Game
{
    ...
    public class GameRoot
    {
        ...
        /// <summary>
        /// Creates and attaches an empty fleet for a faction at a planet.
        /// </summary>
        /// <param name="destination">The planet that receives the fleet.</param>
        ...
        public Fleet CreateFleetAtPlanet(Planet destination, string ownerInstanceId)
        {
            ...
            Faction faction = Factions.FirstOrDefault(candidate =>
            ...
            AttachNode(fleet, destination);
            return fleet;
        }

        /// <summary>
        /// Splits capital ships at one planet into a newly attached fleet.
        /// </summary>
        /// <param name="capitalShips">The capital ships that form the new fleet.</param>
        /// <returns>The attached fleet, or null when the selection is invalid.</returns>
        public Fleet CreateFleetFromCapitalShips(IReadOnlyList<CapitalShip> capitalShips)
        {
            if (capitalShips == null || capitalShips.Count == 0)
                return null;

            List<CapitalShip> ships = capitalShips.Where(ship => ship != null).Distinct().ToList();
            if (ships.Count != capitalShips.Count)
                return null;

            string ownerInstanceId = ships[0].GetOwnerInstanceID();
            ...
            List<Fleet> sourceFleets = new List<Fleet>();
            foreach (CapitalShip ship in ships)
            {
                ...
                if (
                    ...
                    || sourceFleet.GetParentOfType<Planet>() != planet
                )
                    return null;
            ...
            AttachNode(fleet, planet);

            foreach (Fleet sourceFleet in sourceFleets.Distinct())
                RemoveEmptyFleet(sourceFleet);

            return fleet;
        }

        /// <summary>
        /// Removes an attached fleet when it contains no capital ships.
        /// </summary>
        /// <param name="fleet">The fleet to remove when empty.</param>
        /// <returns>True when an empty fleet was removed.</returns>
        public bool RemoveEmptyFleet(Fleet fleet)
        {
        ...
        /// <summary>
        /// Retires owned officers from the active scene graph.
        /// </summary>
        /// <param name="officers">The officers selected for retirement.</param>
        /// <param name="ownerInstanceId">The faction authorized to retire the officers.</param>
        /// <returns>True when every selected officer was retired.</returns>
        public bool RetireOfficers(IReadOnlyList<Officer> officers, string ownerInstanceId)
        {
            if (officers == null || officers.Count == 0 || string.IsNullOrEmpty(ownerInstanceId))
                return false;

            List<Officer> liveOfficers = new List<Officer>(officers.Count);
            foreach (Officer officer in officers)
            {
                Officer liveOfficer = GetSceneNodeByInstanceID<Officer>(officer?.InstanceID);
                if (
                    liveOfficer == null
                    || liveOfficer.GetParent() == null
                    || !string.Equals(
                        liveOfficer.GetOwnerInstanceID(),
                        ownerInstanceId,
                        StringComparison.Ordinal
                    )
                )
                ...
                liveOfficers.Add(liveOfficer);
            }

            foreach (Officer officer in liveOfficers)
                DetachNode(officer);
