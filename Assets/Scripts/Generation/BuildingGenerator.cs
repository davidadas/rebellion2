...
namespace Rebellion.Generation
{
    ...
    public class BuildingGenerator : UnitGenerator<Building>
    {
        ...
        private Dictionary<string, double> GetConfigMapping()
        {
            ...
            if (rules.Buildings == null || rules.Buildings.InitialBuildings == null)
            {
                return mapping;
            }

            foreach (InitialBuildingEntry entry in rules.Buildings.InitialBuildings)
            {
                ...
                if (!mapping.ContainsKey(entry.TypeID))
                {
                    mapping[entry.TypeID] = entry.Frequency;
                }
                else
                {
                    if (entry.Frequency > mapping[entry.TypeID])
                    {
                        mapping[entry.TypeID] = entry.Frequency;
                    }
                }
        ...
        )
        {
            ...
            {
                Building buildingTemplate = availableBuildings.Find(b =>
                    b.TypeID == buildingTypeId
                );
        ...
        )
        {
            return Enumerable
                ...
                .Select(_ =>
                {
                    ...
                    // Generated buildings start at rest (no movement)
                    copy.Movement = null;
                    return copy;
                })
                .ToList();
