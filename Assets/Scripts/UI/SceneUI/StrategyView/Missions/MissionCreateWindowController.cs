...
public sealed class MissionCreateWindowController
{
    ...
    )
    {
        ...
        projector = new MissionCreateWindowProjector(
            getUIContext,
            request => this.gameSession.Queries.GetMissionOdds(request),
            getObservedPlanet
        );
    }
    ...
    )
    {
        ...
        foreach (MissionOption option in gameSession.Queries.GetMissionOptions(context))
    ...
    private bool TryStartMission(MissionCreateWindowSession session)
    {
        ...
        MissionContext context = session.CreateMissionContext(
            choice,
            missionPlanet,
            session.Target.Item
        );
        if (
            !gameSession.Queries.CanCreateMission(context)
            || !gameSession.Execute(new InitiateMissionCommand { Context = context }).Accepted
        )
            return false;
...
internal sealed class MissionCreateWindowSession
{
    ...
    /// <summary>
    /// Builds the mission context shared by planning estimates and final submission.
    /// </summary>
    ...
    /// <returns>A context containing the session's current participant roles.</returns>
    public MissionContext CreateMissionContext(
