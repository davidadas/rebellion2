...
public static class StrategyViewPrefabBuilder
{
    ...
    private const string _optionsMenuWindowPrefabPath =
        "Assets/Prefabs/UI/StrategyView/OptionsMenu.prefab";
    ...
    /// approved mockup (render.py) at 0.584 scale into the 632x474 source-space window.
    /// </summary>
    ...
    private static OptionsMenuView BuildOptionsMenuPrefab()
    {
        ...
        Color accentDim = new Color(0.173f, 0.361f, 0.549f);
        ...
        ConfigureWindowRoot(window.GetComponent<UIWindow>());
        SetSourceRect(window.GetComponent<RectTransform>(), 0, 0, 632, 474);
        ...
        SetSourceRect(header.rectTransform, 50, 28, 220, 28);

        TextMeshProUGUI pageTitle = CreateTextLabel("PageTitleTextField", window.transform);
        pageTitle.text = "GRAPHICS";
        pageTitle.color = accent;
        pageTitle.fontSize = 14;
        pageTitle.alignment = TextAlignmentOptions.MidlineRight;
        ApplyOptionsDisplayFont(pageTitle);
        SetSourceRect(pageTitle.rectTransform, 385, 33, 200, 20);

        GameObject underline = new GameObject(
            "TitleUnderline",
            typeof(RectTransform),
            typeof(CanvasRenderer),
            typeof(Image)
        );
        underline.transform.SetParent(window.transform, false);
        Image underlineImage = underline.GetComponent<Image>();
        underlineImage.color = accentDim;
        underlineImage.raycastTarget = false;
        SetSourceRect(underline.GetComponent<RectTransform>(), 49, 54, 536, 1);
        ...
        for (int i = 0; i < tabNames.Length; i++)
        {
            ...
            TextMeshProUGUI tabLabel = CreateTextLabel("Label", tabButton.transform);
        ...
        RectTransform graphicsPage = CreateChildLayer("GraphicsPage", window.transform);
        SetSourceRect(graphicsPage, 228, 69, 369, 344);
        RectTransform audioPage = CreateChildLayer("AudioPage", window.transform);
        SetSourceRect(audioPage, 228, 69, 369, 344);
        RectTransform saveLoadPage = CreateChildLayer("SaveLoadPage", window.transform);
        SetSourceRect(saveLoadPage, 228, 69, 369, 344);
        RectTransform controlsPage = CreateChildLayer("ControlsPage", window.transform);
        SetSourceRect(controlsPage, 228, 69, 369, 344);

        Button backToGameButton = CreateOptionsFooterButton(
            window.transform,
            "BackToGame",
            "Application/OptionsMenu/UI/ui_settingsmenu_navigationbutton_strategyview_empire",
            "BACK TO GAME",
            239
        );
        Button mainMenuButton = CreateOptionsFooterButton(
            window.transform,
            "MainMenu",
            "Application/OptionsMenu/UI/ui_settingsmenu_navigationbutton_mainmenu",
            "MAIN MENU",
            369
        );
        Button quitButton = CreateOptionsFooterButton(
            window.transform,
            "Quit",
            "Application/OptionsMenu/UI/ui_settingsmenu_navigationbutton_exit",
            "QUIT",
            505
        );
        CreateOptionsFooterDivider(window.transform, "FooterDivider0", 351);
        CreateOptionsFooterDivider(window.transform, "FooterDivider1", 474);
        ...
        SetSourceRect(slotNameTemplate.rectTransform, 12, 4, 300, 14);
        slotNameTemplate.gameObject.SetActive(false);
        TextMeshProUGUI slotMetaTemplate = CreateTextLabel("SlotMetaTemplate", slotContent);
        slotMetaTemplate.text = "Meta";
        slotMetaTemplate.color = textDim;
        slotMetaTemplate.fontSize = 9;
        slotMetaTemplate.alignment = TextAlignmentOptions.TopLeft;
        SetSourceRect(slotMetaTemplate.rectTransform, 12, 17, 300, 10);
        ...
        SetSourceRect(bindingActionTemplate.rectTransform, 11, 4, 215, 15);
        ...
        SetSourceRect(bindingKeyTemplate.rectTransform, 281, 3, 61, 16);
    ...
    /// <summary>
    /// Authors one footer navigation button with an ornate icon and caps label.
    /// </summary>
    /// <param name="parent">The window transform.</param>
    /// <param name="name">The button object name.</param>
    /// <param name="artAddress">The icon content address.</param>
    /// <param name="label">The caps button caption.</param>
    /// <param name="x">The source-space group left.</param>
    /// <returns>The configured button.</returns>
    private static Button CreateOptionsFooterButton(
        Transform parent,
        string name,
        string artAddress,
        string label,
    ...
    {
        RawImage icon = CreateRawImage(name + "Icon", parent, artAddress, x, 424, 29, 29);
        Button button = CreateButton(icon);
        TextMeshProUGUI text = CreateTextLabel(name + "Label", parent);
        text.text = label;
        text.color = new Color(0.875f, 0.910f, 0.941f);
        text.fontSize = 10;
        text.alignment = TextAlignmentOptions.MidlineLeft;
        SetSourceRect(text.rectTransform, x + 36, 424, 80, 29);
        return button;
    }

    /// <summary>
    /// Authors one thin vertical divider between footer navigation slots.
    /// </summary>
    /// <param name="parent">The window transform.</param>
    /// <param name="name">The divider object name.</param>
    /// <param name="x">The source-space divider position.</param>
    private static void CreateOptionsFooterDivider(Transform parent, string name, int x)
    {
        ...
        image.color = new Color(0.173f, 0.361f, 0.549f, 0.63f);
        image.raycastTarget = false;
        SetSourceRect(divider.GetComponent<RectTransform>(), x, 427, 1, 24);
    }
    ...
    )
    {
        ...
        prevButton = CreateOptionsStepperButton(parent, $"{name}Prev", "<", 194, y);
        ...
        nextButton = CreateOptionsStepperButton(parent, $"{name}Next", ">", 323, y);
        return valueField;
    }
    ...
    )
    {
        ...
        hitArea.color = Color.clear;
        hitArea.raycastTarget = true;
        SetSourceRect(hotspot.GetComponent<RectTransform>(), x, y, 26, 18);
        ...
        SetSourceRect(glyphText.rectTransform, 0, 0, 26, 18);
        return button;
    }
    ...
    )
    {
        ...
        SetSourceRect(toggle.GetComponent<RectTransform>(), 174, 0, 30, 14);
        ...
        stateField.alignment = TextAlignmentOptions.MidlineLeft;
        SetSourceRect(stateField.rectTransform, 211, 0, 60, 14);
    ...
    /// <summary>
    /// Authors one 9-sliced rounded panel image (navy chrome) at a source-space rectangle.
    /// </summary>
    ...
    /// <summary>
    /// Authors one 9-sliced rounded button (navy chrome) at a source-space rectangle.
    /// </summary>
    /// <param name="name">The button object name.</param>
    /// <param name="parent">The parent transform.</param>
    /// <param name="spritePath">The chrome sprite asset path.</param>
    ...
    )
    {
        ...
        AssignWindowPrefab(target, "optionsMenuWindowPrefab", optionsMenuWindowPrefab);
    }
