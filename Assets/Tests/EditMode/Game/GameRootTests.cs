...
namespace Rebellion.Tests.Game
{
    ...
    public class GameRootTests
    {
        ...
        [Test]
        public void CreateFleetFromCapitalShips_SplitsSelectedShipIntoNewFleet()
        {
            GameRoot game = CreateFleetTestGame();
            Planet planet = CreateFleetTestPlanet(game, "p1", "empire");
            CapitalShip selectedShip = CreateFleetTestCapitalShip("ship1", "empire");
            CapitalShip remainingShip = CreateFleetTestCapitalShip("ship2", "empire");
            Fleet sourceFleet = CreateFleetTestFleet(
                game,
                planet,
                "empire",
                selectedShip,
            ...
            Fleet createdFleet = game.CreateFleetFromCapitalShips(
                new List<CapitalShip> { selectedShip }
            );

            Assert.IsNotNull(createdFleet);
            Assert.AreEqual(2, planet.GetFleets().Count);
            Assert.Contains(sourceFleet, planet.GetFleets());
            Assert.Contains(createdFleet, planet.GetFleets());
            Assert.AreSame(createdFleet, selectedShip.GetParent());
            Assert.AreSame(sourceFleet, remainingShip.GetParent());
            Assert.AreEqual(1, sourceFleet.CapitalShips.Count);
            Assert.AreEqual(1, createdFleet.CapitalShips.Count);
        }

        [Test]
        public void CreateFleetFromCapitalShips_RemovesEmptySourceFleet()
        {
            GameRoot game = CreateFleetTestGame();
            Planet planet = CreateFleetTestPlanet(game, "p1", "empire");
            CapitalShip selectedShip = CreateFleetTestCapitalShip("ship1", "empire");
            Fleet sourceFleet = CreateFleetTestFleet(game, planet, "empire", selectedShip);
            ...
            Assert.IsNotNull(createdFleet);
            Assert.AreEqual(1, planet.GetFleets().Count);
            Assert.AreSame(createdFleet, planet.GetFleets()[0]);
            Assert.AreSame(createdFleet, selectedShip.GetParent());
            Assert.IsNull(sourceFleet.GetParent());
        }

        [Test]
        public void RetireOfficers_OwnedAttachedOfficer_RemovesOfficer()
        {
            GameRoot game = CreateFleetTestGame();
            Planet planet = CreateFleetTestPlanet(game, "p1", "empire");
            Officer officer = new Officer { InstanceID = "officer1", OwnerInstanceID = "empire" };
            game.AttachNode(officer, planet);

            bool retired = game.RetireOfficers(new List<Officer> { officer }, "empire");

            Assert.IsTrue(retired);
            Assert.IsNull(game.GetSceneNodeByInstanceID<Officer>(officer.InstanceID));
            Assert.IsNull(officer.GetParent());
        }

        [Test]
        public void RetireOfficers_OtherFactionOfficer_PreservesOfficer()
        {
            GameRoot game = CreateFleetTestGame();
            Planet planet = CreateFleetTestPlanet(game, "p1", "empire");
            Officer officer = new Officer { InstanceID = "officer1", OwnerInstanceID = "empire" };
            ...
            bool retired = game.RetireOfficers(new List<Officer> { officer }, "alliance");
        ...
        private static GameRoot CreateFleetTestGame()
        {
        ...
        private static Planet CreateFleetTestPlanet(
            GameRoot game,
            string planetId,
            string factionId
        )
        ...
        private static CapitalShip CreateFleetTestCapitalShip(string instanceId, string factionId)
        {
        ...
        private static Fleet CreateFleetTestFleet(
            GameRoot game,
            Planet planet,
            string factionId,
        ...
        {
            Fleet fleet = new Fleet(factionId, $"{planet.InstanceID}_fleet");
