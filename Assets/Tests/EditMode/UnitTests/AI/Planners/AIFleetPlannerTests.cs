...
namespace Rebellion.Tests.AI.Planners
{
    ...
    public class AIFleetPlannerTests
    {
        ...
        /// </summary>
        [Test]
        public void Plan_WithUntypedMixedFleet_ExtractsOnlyDedicatedTransport()
        {
            ...
            warship.Roles.Add(CapitalShipRole.PrimaryLine);
            game.AttachNode(fleet, owned);
            game.AttachNode(transport, fleet);
            game.AttachNode(warship, fleet);
            AITurnContext context = AITestSceneBuilder.CreateContext(game, empire);

            AIFleetRoleProposal proposal = new AIFleetPlanner()
                .Plan(context)
                .OfType<AIFleetRoleProposal>()
                .Single(item => item.Fleet == fleet);

            Assert.AreEqual(FleetRoleType.Colonization, proposal.Role);
        ...
        public void Plan_WithUntypedTroopCarryingWarship_AssignsBattleRole()
        {
            ...
            Assert.AreEqual(FleetRoleType.Battle, proposal.Role);
        ...
        /// <summary>
        /// Verifies an idle fleet unable to attack its hostile location returns to friendly
        /// territory even when it has no existing order.
        /// </summary>
        [Test]
        public void Plan_WithIdleFleetAtUnattackableEnemyPlanet_ReturnsFleetToFriendlyTerritory()
        {
            GameRoot game = AITestSceneBuilder.CreateGame(out Faction empire, out Faction rebels);
            PlanetSector system = AITestSceneBuilder.AddSector(game, "system");
            ...
            AddShield(game, hostile, "shield-1", rebels.InstanceID);
            AddShield(game, hostile, "shield-2", rebels.InstanceID);
            AITestSceneBuilder.RevealPlanet(game, empire, hostile);
            Fleet fleet = AddBattleFleet(game, hostile, empire.InstanceID, "fleet");
            AITurnContext context = AITestSceneBuilder.CreateContext(game, empire);

            AIFleetEvacuationProposal proposal = new AIFleetPlanner()
                .Plan(context)
                .OfType<AIFleetEvacuationProposal>()
                .Single(candidate => candidate.Fleet == fleet);

            Assert.AreSame(hostile, proposal.HostilePlanet);
        }

        /// <summary>
        /// Verifies an unusable assignment does not prevent a stranded fleet from leaving hostile
        /// orbit.
        /// </summary>
        /// <param name="orderType">Order type assigned to the stranded fleet.</param>
        [TestCase(FleetOrderType.Attack)]
        [TestCase(FleetOrderType.Colonize)]
        [TestCase(FleetOrderType.Defend)]
        [TestCase(FleetOrderType.Engage)]
        public void Plan_WithAssignedFleetAtUnattackableEnemyPlanet_ReturnsFleetToFriendlyTerritory(
        ...
        )
        {
            ...
            fleet.Order = new FleetOrder
            {
                ...
                TargetPlanetId = hostile.InstanceID,
            };
            AITurnContext context = AITestSceneBuilder.CreateContext(game, empire);
        ...
        public void Plan_WithShieldBlockedAttackAtHostileTarget_ReturnsWithoutRetargeting()
        {
            ...
            List<AIFleetEvacuationProposal> proposals = new AIFleetPlanner()
                .Plan(context)
                .OfType<AIFleetEvacuationProposal>()
                .Where(proposal => proposal.Fleet == fleet)
                .ToList();
            ...
            Assert.AreEqual(1, proposals.Count);
            Assert.AreSame(blockedTarget, proposals[0].HostilePlanet);
        }
        ...
        public void Plan_WithStalledAttackAndNoAlternative_ReturnsFleetToFriendlyTerritory()
        {
            ...
            Assert.AreSame(target, proposal.HostilePlanet);
        }
        ...
        public void Plan_WithIncapableAttackFleetAtHostileNonTarget_ReturnsFleetToFriendlyTerritory()
        {
            ...
            Assert.AreSame(hostileStaging, proposal.HostilePlanet);
        }
        ...
        public void Plan_WithDecisivePlanetAdvantageAndMobileEnemyHeadquarters_AddsAttackProposal()
        {
            ...
            PlanetSector system = AITestSceneBuilder.AddSector(game, "system");
            Planet owned = AITestSceneBuilder.AddPlanet(game, system, "owned", empire.InstanceID);
            AITestSceneBuilder.AddPlanet(game, system, "owned-2", empire.InstanceID);
            AITestSceneBuilder.AddPlanet(game, system, "owned-3", empire.InstanceID);
            Planet enemy = AITestSceneBuilder.AddPlanet(game, system, "enemy", rebels.InstanceID);
            rebels.Settings.Headquarters.IsMobile = true;
            rebels.HQInstanceID = null;
            AITestSceneBuilder.RevealPlanet(game, empire, enemy);
            Fleet fleet = AddBattleFleet(game, owned, empire.InstanceID, "fleet");
            AITurnContext context = AITestSceneBuilder.CreateContext(game, empire);
            ...
            Assert.IsTrue(
                proposals.Any(proposal =>
                    proposal.Fleet == fleet && proposal.TargetPlanet.InstanceID == enemy.InstanceID
                )
            );
        }
        ...
        /// <summary>
        /// Verifies a colonization fleet stranded at a hostile planet evacuates before continuing
        /// its survey campaign.
        /// </summary>
        [Test]
        public void Plan_WithColonizationFleetStrandedAtHostilePlanet_AddsEvacuationProposal()
        {
            GameRoot game = AITestSceneBuilder.CreateGame(out Faction empire, out Faction rebels);
            ...
            AITestSceneBuilder.AddPlanet(game, system, "unexplored", null).IsColonized = false;
            AITestSceneBuilder.RevealPlanet(game, empire, hostile);
            Fleet fleet = AddBattleFleet(game, hostile, empire.InstanceID, "fleet");
            fleet.RoleType = FleetRoleType.Colonization;
            fleet.Order = new FleetOrder
        ...
        /// </summary>
        [Test]
        public void Plan_WithUnderstrengthHeadquartersOrder_ReinforcesAssignedFleet()
        {
            GameRoot game = AITestSceneBuilder.CreateGame(out Faction empire, out Faction _);
            game.Config.AI.FleetDeployment.MinimumDefenseStrength = 1000;
            PlanetSector system = AITestSceneBuilder.AddSector(game, "system");
            ...
            List<AIFleetDefenseProposal> proposals = new AIFleetPlanner()
                .Plan(context)
                .OfType<AIFleetDefenseProposal>()
                .ToList();
            ...
            Assert.IsTrue(
                proposals.Any(candidate =>
                    candidate.Fleet == assignedFleet && candidate.TargetPlanet == headquarters
                )
            );
        ...
        public void Plan_WithMultipleHeadquartersDefenseOrders_ClearsWeakerOrder()
        {
            ...
            headquarters.IsHeadquarters = true;
            empire.HQInstanceID = headquarters.InstanceID;
            Fleet weakerFleet = AddBattleFleet(
                game,
                headquarters,
                empire.InstanceID,
                "weaker-fleet",
            ...
            Fleet strongerFleet = AddBattleFleet(
                game,
                headquarters,
                empire.InstanceID,
                "stronger-fleet",
            ...
            weakerFleet.Order = new FleetOrder
            {
                OrderType = FleetOrderType.Defend,
                Status = FleetOrderStatus.Ready,
                TargetPlanetId = headquarters.InstanceID,
            };
            strongerFleet.Order = new FleetOrder
            {
                OrderType = FleetOrderType.Defend,
                Status = FleetOrderStatus.Ready,
                TargetPlanetId = headquarters.InstanceID,
            ...
            List<AIProposal> proposals = new AIFleetPlanner().Plan(
            ...
            Assert.IsTrue(
                proposals
                    .OfType<AIFleetDefenseProposal>()
                    .Any(candidate =>
                        candidate.Fleet == strongerFleet && candidate.TargetPlanet == headquarters
                    )
            );
            Assert.IsTrue(
                proposals
                    .OfType<AIClearFleetOrderProposal>()
                    .Any(candidate => candidate.Fleet == weakerFleet)
            );
        }
        ...
        public void Plan_WithAttackFleetMissingRegiment_SourcesFromMostSecurePlanet()
        {
            ...
            Planet lessSecure = AITestSceneBuilder.AddPlanet(
                game,
                system,
                "less-secure",
                empire.InstanceID
            );
            Planet secure = AITestSceneBuilder.AddPlanet(game, system, "secure", empire.InstanceID);
            Planet target = AITestSceneBuilder.AddPlanet(game, system, "target", rebels.InstanceID);
            lessSecure.SetPopularSupport(empire.InstanceID, 70);
            secure.SetPopularSupport(empire.InstanceID, 100);
            target.SetPopularSupport(empire.InstanceID, 100);
            AITestSceneBuilder.RevealPlanet(game, empire, target);
            Regiment lessSecureRegiment = AITestSceneBuilder.CreateRegiment(
                "less-secure-regiment",
                empire.InstanceID
            );
            Regiment secureRegiment = AITestSceneBuilder.CreateRegiment(
                "secure-regiment",
                empire.InstanceID
            );
            game.AttachNode(lessSecureRegiment, lessSecure);
            game.AttachNode(secureRegiment, secure);
            Fleet fleet = AddBattleFleet(
                game,
                staging,
                empire.InstanceID,
            ...
            AITransferUnitProposal proposal = new AIFleetPlanner()
                .Plan(context)
                .OfType<AITransferUnitProposal>()
                .Single(candidate => candidate.Unit is Regiment);

            Assert.AreSame(secure, proposal.SourceContainer);
            Assert.AreSame(secureRegiment, proposal.Unit);
        ...
        public void Plan_WithEquallySecureRegimentSources_PrefersNearestPlanet()
        {
            ...
            staging.PositionX = 0;
            Planet near = AITestSceneBuilder.AddPlanet(game, system, "near", empire.InstanceID);
            near.PositionX = 10;
            Planet far = AITestSceneBuilder.AddPlanet(game, system, "far", empire.InstanceID);
            far.PositionX = 100;
            Planet target = AITestSceneBuilder.AddPlanet(game, system, "target", rebels.InstanceID);
            near.SetPopularSupport(empire.InstanceID, 100);
            far.SetPopularSupport(empire.InstanceID, 100);
            target.SetPopularSupport(empire.InstanceID, 100);
            AITestSceneBuilder.RevealPlanet(game, empire, target);
            Regiment nearRegiment = AITestSceneBuilder.CreateRegiment(
                "near-regiment",
                empire.InstanceID
            );
            game.AttachNode(nearRegiment, near);
            game.AttachNode(
                AITestSceneBuilder.CreateRegiment("far-regiment", empire.InstanceID),
                far
            );
            Fleet fleet = AddBattleFleet(
                game,
            ...
            Assert.AreSame(near, proposal.SourceContainer);
        ...
        /// <returns>The result of add donor fleet.</returns>
