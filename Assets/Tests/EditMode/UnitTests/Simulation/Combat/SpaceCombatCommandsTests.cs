...
{
    /// <summary>
    /// Tests strategy-level space combat detection, application, and reporting.
    /// </summary>
    [TestFixture]
    public class SpaceCombatCommandsTests : CombatTestBase
    {
        ...
        public void Resolve_CompletedEncounter_ReturnsAggregateDamage()
        {
            ...
            foreach (ShipDamageResult damage in combatResult.ShipDamage)
            {
                int reportedDamage = combatResult
                    .Events.OfType<GameObjectDamagedResult>()
                    .Where(result => result.GameObject == damage.Ship)
                    .Sum(result => result.DamageValue);
                Assert.AreEqual(100, damage.HullBefore);
                Assert.AreEqual(damage.Ship.CurrentHullStrength, damage.HullAfter);
                Assert.AreEqual(damage.HullBefore - damage.HullAfter, reportedDamage);
            }
        }
        ...
        )
        {
            ...
            Fleet destroyedFleet = attackerCanEvacuate ? defender : attacker;
        ...
        public void ProcessTick_FleetWithdrawalInterruptedByVictory_KeepsEntireFleetAtCombatPlanet()
        {
            ...
            CreatePlanet(game, "alliance-fallback", owner: "alliance");
            CreateFleet(game, "empire-fleet", "empire", combatPlanet, 1, 1, 0);
            Fleet allianceFleet = CreateFleet(
                game,
                "alliance-fleet",
                "alliance",
            ...
            IReadOnlyList<CapitalShip> allianceShips = allianceFleet.GetChildren<CapitalShip>();
            allianceShips[0].SublightSpeed = 20;
            allianceShips[1].SublightSpeed = 10;
            Starfighter coveringFighter = new Starfighter
            {
                InstanceID = "covering-fighter",
                OwnerInstanceID = "alliance",
                ManufacturingStatus = ManufacturingStatus.Complete,
                MaxSquadronSize = 1,
        ...
        public void ResolvePending_PlanetaryHyperdriveFightersReachWithdrawalThreshold_WithdrawsFighters()
        {
            ...
            attackerFleet.GetChildren<CapitalShip>().Single().WeaponRecharge = 9;
        ...
        {
            SpaceCombatCommands commands = MakeSpaceCombat(CreateGame());
        ...
        public void ResolvePendingRetreat_NoPendingEncounter_ThrowsInvalidOperationException()
        {
            ...
            Assert.Throws<InvalidOperationException>(() =>
                commands.ResolvePendingRetreat("empire")
            );
        }
        ...
        public void ResolvePendingRetreat_UnrelatedFaction_KeepsPendingDecision()
        {
            ...
            List<GameResult> results = commands.ResolvePendingRetreat("unrelated");

            Assert.IsNull(results);
            Assert.IsTrue(commands.HasPendingDecision);
            Assert.IsTrue(empireFleet.IsInCombat);
            Assert.IsTrue(allianceFleet.IsInCombat);
            Assert.IsNull(empireFleet.Movement);
            Assert.IsNull(allianceFleet.Movement);
        }
        ...
        public void ResolvePending_ManualResolution_ClearsPendingWithoutDamage()
        {
            ...
            List<GameResult> results = commands.ResolvePending(false);

            Assert.IsEmpty(results);
            Assert.IsFalse(commands.HasPendingDecision);
            Assert.IsFalse(empireFleet.IsInCombat);
            Assert.IsFalse(allianceFleet.IsInCombat);
            Assert.AreEqual(100, empireFleet.GetChildren<CapitalShip>()[0].CurrentHullStrength);
        ...
        public void TryGetPendingCombat_GravityWellActivated_RefreshesRetreatAvailability()
        {
            (GameRoot game, SpaceCombatCommands commands, Fleet empireFleet, Fleet allianceFleet) =
                CreatePendingEncounter(failResolution: true);
            Assert.IsTrue(commands.TryGetPendingCombat(out PendingCombatResult before));
            bool couldRetreat =
                before.AttackerFleet == empireFleet
                    ? before.AttackerCanRetreat
                    : before.DefenderCanRetreat;
            Assert.IsTrue(couldRetreat);
            allianceFleet.GetChildren<CapitalShip>()[0].HasGravityWell = true;
            game.CurrentTick = 42;

            Assert.IsTrue(commands.TryGetPendingCombat(out PendingCombatResult after));

            Assert.AreNotSame(before, after);
            Assert.IsFalse(
                after.AttackerFleet == empireFleet
                    ? after.AttackerCanRetreat
                    : after.DefenderCanRetreat
            );
            Assert.AreEqual(42, after.Tick);
            Assert.IsTrue(commands.HasPendingDecision);
            Assert.IsNull(empireFleet.Movement);
        }

        /// <summary>
        /// Creates a pending human encounter with evacuation destinations for both factions.
        /// </summary>
        /// <param name="failResolution">Whether all combat rolls should throw.</param>
        /// <returns>The game, pending-combat owner, and opposing fleets.</returns>
        private (
            GameRoot game,
            SpaceCombatCommands commands,
            Fleet empireFleet,
            Fleet allianceFleet
        ) CreatePendingEncounter(bool failResolution = false)
        {
            GameRoot game = CreateGame();
            game.Random = failResolution ? new ThrowingRNG() : new SequenceRNG();
            game.SetFactionController("empire", "player1", PlayerControllerType.Human);
            ...
            Fleet empireFleet = CreateFleet(game, "empire-fleet", "empire", planet, 1, 100, 10);
            Fleet allianceFleet = CreateFleet(
                game,
                "alliance-fleet",
                "alliance",
        ...
        /// <returns>True when the damage for condition is met; otherwise false.</returns>
        ...
        /// </summary>
        /// <param name="results">The results.</param>
        /// <returns>The requested damage results.</returns>
        ...
        /// <summary>
        /// Resolves carrier destruction withdrawal.
        /// </summary>
        /// <param name="recoveryCapacity">The recovery capacity.</param>
        /// <param name="displacedFighterHyperdrives">The displaced fighter hyperdrives.</param>
        ...
        /// <summary>
        /// Creates automatic combat game.
        /// </summary>
        /// <returns>The created automatic combat game.</returns>
        ...
        /// <summary>
        /// Checks whether the opposing ready fleets condition is met.
        /// </summary>
        /// <param name="planet">The planet.</param>
        /// <returns>True when the opposing ready fleets condition is met; otherwise false.</returns>
        ...
        /// <summary>
        /// Checks whether the hostile fleets condition is met.
        /// </summary>
        /// <param name="planet">The planet.</param>
        /// <returns>True when the hostile fleets condition is met; otherwise false.</returns>
        ...
        /// <param name="hullStrength">The hull strength.</param>
        /// <param name="weaponPower">The weapon power.</param>
        /// <param name="squadronSize">The squadron size.</param>
        /// <returns>The created fleet with fighters.</returns>
