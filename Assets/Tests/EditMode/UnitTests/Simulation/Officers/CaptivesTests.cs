...
namespace Rebellion.Tests.Simulation.OfficersTests
{
    [TestFixture]
    public class CaptivesTests
    {
        /// <summary>
        /// Verifies authored capture commands update captivity and emit the standard fact.
        /// </summary>
        [Test]
        public void HandleCommands_CaptureCommand_UpdatesOfficerAndReturnsStateChange()
        {
            (GameRoot game, Planet _, Officer captive, SimulationMovement movement) = BuildScene();
            Captives captives = CreateSystem(game, new FixedRNG(0.0), movement);

            List<GameResult> results = captives.HandleCommands(
                new[]
                {
                    new SetCaptureStatusCommand
                    {
                        Officers = new List<Officer> { captive },
                        IsCaptured = true,
            ...
            Assert.AreEqual("empire", captive.CaptorInstanceID);
            Assert.IsFalse(captive.CanEscape);
            OfficerCaptureStateResult result = results.OfType<OfficerCaptureStateResult>().Single();
            Assert.AreSame(captive, result.TargetOfficer);
            Assert.AreEqual(12, result.Tick);
        }

        [Test]
        public void HandleResults_CaptureAtCaptorPlanet_RecordsImmediateCustody()
        {
            (GameRoot game, Planet planet, Officer captive, SimulationMovement movement) =
                BuildScene();
            Captives system = CreateSystem(game, new FixedRNG(0.0), movement);
        ...
        {
            (GameRoot game, Planet destination, Officer captive, SimulationMovement movement) =
                BuildScene();
        ...
        public void ProcessTick_UnscheduledCaptiveUsesMaximumRoll_SchedulesMaximumInterval()
        {
            (GameRoot game, Planet _, Officer captive, SimulationMovement movement) = BuildScene();
            captive.NextEscapeAttemptTick = 0;
            Captives system = CreateSystem(game, new MaximumRNG(), movement);
        ...
        public void ProcessTick_EscapeAttemptNotDue_SkipsEscapeRoll()
        {
            ...
            Captives system = CreateSystem(game, new ThrowingRNG(), movement);
        ...
        {
            (GameRoot game, Planet planet, Officer dueCaptive, SimulationMovement movement) =
                BuildScene();
        ...
        public void ProcessTick_EscapeRollFails_ReschedulesEscapeAttempt()
        {
            ...
            Captives system = CreateSystem(game, new FixedRNG(0.99), movement);
        ...
        public void ProcessTick_StrongGarrison_LowerEscapeChance()
        {
            ...
            Captives system = CreateSystem(game, new FixedRNG(0.5), movement);
        ...
        public void ProcessTick_NoGarrison_HigherEscapeChance()
        {
            ...
            Captives system = CreateSystem(game, new FixedRNG(0.2), movement);
        ...
        private static Captives CreateSystem(
            GameRoot game,
            IRandomNumberProvider provider,
            SimulationMovement movement
        )
        {
            return new Captives(game, provider, movement, new FogOfWar(game));
        }
        ...
        ) BuildScene()
        {
            ...
            SimulationMovement movement = new SimulationMovement(
                game,
                new FogOfWar(game),
                new Fleets(game)
            );
