...
namespace Rebellion.Tests.Simulation
{
    ...
    {
        private static readonly Dictionary<Message, MessageDelivery> _deliveriesByMessage =
        ...
        {
            Message content = new StatusMessage(MessageType.Conflict, "Captured", "Resolved report")
            {
                ResultType = MessageResultType.OfficerCaptured,
                BackgroundImageKey = "mission_report",
                DisplayImagePath = "background",
                OverlayImagePath = "officer",
                BackgroundAudioPath = "audio",
                OfficerVoicePath = "voice",
                EventLocationInstanceID = "planet",
                NavigationTargetInstanceID = "target",
                NavigationSecondaryTargetInstanceID = "secondary",
                MissionInstanceID = "mission",
            };
            MessageDelivery delivery = new()
            {
                Message = content,
                ExistingMessage = new CombatReport(),
            };

            Message message = new MessageFactory(null).CreateMessage(delivery);

            Assert.AreSame(delivery.ExistingMessage, message);
            Assert.AreEqual(content.Type, message.Type);
            Assert.AreEqual(content.ResultType, message.ResultType);
            Assert.AreEqual(content.Title, message.Title);
            Assert.AreEqual(content.Title, message.DisplayName);
            Assert.AreEqual(content.Body, message.Body);
            Assert.AreEqual(content.BackgroundImageKey, message.BackgroundImageKey);
            Assert.AreEqual(content.DisplayImagePath, message.DisplayImagePath);
            Assert.AreEqual(content.OverlayImagePath, message.OverlayImagePath);
            Assert.AreEqual(content.BackgroundAudioPath, message.BackgroundAudioPath);
            Assert.AreEqual(content.OfficerVoicePath, message.OfficerVoicePath);
            Assert.AreEqual(content.EventLocationInstanceID, message.EventLocationInstanceID);
            Assert.AreEqual(content.NavigationTargetInstanceID, message.NavigationTargetInstanceID);
            Assert.AreEqual(
        ...
        {
            Message content = new StatusMessage(MessageType.Mission, "Prepared", "Report");

            Message message = new MessageFactory(null).CreateMessage(
                new MessageDelivery { Message = content }
            );
        ...
        {
            Faction recipient = new() { InstanceID = "recipient" };
            MessageDelivery delivery = new()
            {
                Recipient = recipient,
        ...
        {
            CombatReportUnit unit = new() { InstanceID = "ship", Destroyed = true };
            CombatReport report = new()
            {
                InstanceID = "report",
                CombatType = CombatReportType.SpaceBattle,
                PlanetName = "Yavin",
                AttackingUnits = new List<CombatReportUnit> { unit },
            };
            ...
            Assert.AreSame(report, message);
            Assert.AreEqual("report", report.InstanceID);
            Assert.AreEqual(CombatReportType.SpaceBattle, report.CombatType);
            Assert.AreEqual("Yavin", report.PlanetName);
        ...
        {
            CombatReport report = new() { CreatedTick = 17 };

            new MessageFactory(null).CreateMessage(
                new MessageDelivery { ExistingMessage = report, Message = new StatusMessage() }
            );
        ...
        public void CreateMessage_EmptyPresentation_ClearsSuppliedReportFields()
        {
            CombatReport report = new()
            {
                ...
                DisplayImagePath = "old-image",
                OverlayImagePath = "old-overlay",
                BackgroundAudioPath = "old-audio",
                OfficerVoicePath = "old-voice",
                EventLocationInstanceID = "old-location",
                NavigationTargetInstanceID = "old-target",
                NavigationSecondaryTargetInstanceID = "old-secondary",
                MissionInstanceID = "old-mission",
            };
            ...
            Assert.IsNull(report.Body);
            Assert.IsNull(report.DisplayName);
            Assert.IsNull(report.BackgroundImageKey);
            Assert.IsNull(report.DisplayImagePath);
            Assert.IsNull(report.OverlayImagePath);
            Assert.IsNull(report.BackgroundAudioPath);
            Assert.IsNull(report.OfficerVoicePath);
            Assert.IsNull(report.EventLocationInstanceID);
            Assert.IsNull(report.NavigationTargetInstanceID);
            Assert.IsNull(report.NavigationSecondaryTargetInstanceID);
        ...
        public void CreateAuthoredMessage_TemplatePreparation_DoesNotConsumeInstanceIdentifiers()
        {
            try
            {
                BaseGameEntity.SetInstanceIdSeed(53);
                string expected = new StatusMessage().InstanceID;
                BaseGameEntity.SetInstanceIdSeed(53);
                MessageFactory factory = new(null);

                factory.CreateAuthoredMessage(
                    new MessageDefinition { Subject = "Prepared" },
                    new Faction { InstanceID = "recipient" }
                );

                Assert.AreEqual(expected, new StatusMessage().InstanceID);
            }
            finally
            {
        ...
        public void CreateAuthoredMessage_ExplicitContext_UsesDataDefinedPresentation()
        {
            ...
            MessageDelivery delivery = factory.CreateAuthoredMessage(
                new MessageDefinition
                {
                    ...
                    BackgroundImage = new MessageBackgroundImage { Path = "Story/image" },
                ...
                destination,
                "Officers/luke",
                new AdvisorNotification { Preset = AdvisorNotificationPreset.SubjectReport },
                "STORY_EVENT"
            );
        ...
        public void CreateMessages_SpaceBattle_RetainsDetachedCompletedOutcome()
        {
            ...
            CombatReport report = FirstMessageFor(deliveries, alliance) as CombatReport;
        ...
        public void CreateMessages_Bombardment_UsesOwnershipAndLossSelectors()
        {
            ...
            CombatReport report = defendingMessage as CombatReport;
        ...
        public void CreateMessages_PlanetaryAssault_UsesOwnershipAndOutcomeSelectors()
        {
            ...
            CombatReport report = defenderMessage as CombatReport;
        ...
        private static MessageDelivery DeliveryFor(Message message) =>
            _deliveriesByMessage[message];

        /// <summary>
        /// Materializes each prepared delivery once through the production factory.
        /// </summary>
        /// <param name="request">The request.</param>
        /// <returns>The result of as message.</returns>
        private static Message AsMessage(MessageDelivery request)
        {
            if (_messagesByDelivery.TryGetValue(request, out Message existing))
                return existing;

            Message message = new MessageFactory(null).CreateMessage(request);
