...
namespace Rebellion.Tests.Simulation.GalaxyTests
{
    [TestFixture]
    public class PlanetaryControlTests
    {
        ...
        private SimulationMovement _movementSystem;
        private PlanetaryControl _ownershipSystem;
        ...
        public void SetUp()
        {
            ...
            _movementSystem = new SimulationMovement(_game, new FogOfWar(_game), new Fleets(_game));
            _ownershipSystem = new PlanetaryControl(
                _game,
                _movementSystem,
                new Manufacturing(_game, new Fleets(_game)),
                new FogOfWar(_game)
            );
        }

        /// <summary>
        /// Verifies authored support commands rebalance support and report every changed faction.
        /// </summary>
        [Test]
        public void HandleCommands_SupportIncrease_RebalancesAndReturnsChangedValues()
        {
            _targetPlanet.SetPopularSupport(_empire.InstanceID, 60);
            _targetPlanet.SetPopularSupport(_rebels.InstanceID, 40);

            List<GameResult> results = _ownershipSystem.HandleCommands(
                new[]
                {
            ...
            Assert.AreEqual(70, _targetPlanet.GetPopularSupport(_empire.InstanceID));
            Assert.AreEqual(30, _targetPlanet.GetPopularSupport(_rebels.InstanceID));
            Assert.AreEqual(2, results.OfType<PlanetStatChangedResult>().Count());
            Assert.IsTrue(results.All(result => result.Tick == 4));
        }
        ...
        {
            (Planet planet, PlanetaryControl system) = BuildSupportScene(support: 15);
        ...
        {
            (Planet planet, PlanetaryControl system) = BuildSupportScene(
                support: 59,
                ownerInstanceId: null
            );
        ...
        private static (Planet planet, PlanetaryControl system) BuildSupportScene(
            int support,
        ...
        )
        {
            ...
            FogOfWar fogOfWarSystem = new FogOfWar(game);
            ...
            PlanetaryControl controlSystem = new PlanetaryControl(
                game,
                movementSystem,
                new Manufacturing(game, fleetSystem),
                fogOfWarSystem
            );
        ...
        private void CapturePlanetSnapshot(Faction faction, Planet planet, int tick)
        {
            ...
            new FogOfWar(_game).CaptureSnapshot(faction, planet, planetSector, tick);
        }
